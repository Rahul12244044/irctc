...
import sendConfirmationEmail from "../services/email.jsx";
...
function BookingPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const emailData = {
      to_name: currentUser?.displayName || "Passenger",
      to_email: contactInfo.email,
      train_name: trainDetails.trainName,
      train_number: trainDetails.trainNumber,
      from: trainDetails.from,
      to: trainDetails.to,
      journey_date: trainDetails.date,
      class: selectedClass,
      booking_id: docRef.id,
      passenger_list: passengers.map(
        (p, i) => `Passenger ${i + 1}: ${p.name}, Age: ${p.age}, Gender: ${p.gender}, Berth: ${p.berth}`
      ).join("\n"),
      total_fare: calculateTotalFare().total,
    };
      sendConfirmationEmail(emailData); // 📧 Send the emai
